...
generateMaterialDesignPalette(url, (error, palette) => {
  if (error) {
    ...
    document.getElementById("back").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    ...
    document.getElementById("filters").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("blur").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("brightness").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("contrast").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("grayscale").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("hue-rotate").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("invert").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("opacity").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("saturate").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    document.getElementById("sepia").style.backgroundColor = generateRGBA(
      palette.accent,
      0.25
    );
    ...
    document.getElementById("brightness").addEventListener("focusin", () => {
      document.getElementById("brightness").style.backgroundColor =
    ...
    document.getElementById("hue-rotate").addEventListener("focusin", () => {
      document.getElementById("hue-rotate").style.backgroundColor =
        generateRGBA(palette.accent, 0.5);
    });
...
function addFilter(filter) {
  if (filter == "blur") {
    ...
    if (!img.style.filter.includes("blur")) {
      img.style.filter += `blur(${
        document.getElementById("blur-input").value
      }px)`;
    } else {
      img.style.filter = img.style.filter.replace(/blur\([^)]*\)/, "");
      img.style.filter += `blur(${
    ...
    if (!img.style.filter.includes("grayscale")) {
      img.style.filter += `grayscale(${
        document.getElementById("grayscale-input").value / 100
      })`;
    } else {
      img.style.filter = img.style.filter.replace(/grayscale\([^)]*\)/, "");
      img.style.filter += `grayscale(${
    ...
    if (!img.style.filter.includes("hue-rotate")) {
      img.style.filter += `hue-rotate(${
        document.getElementById("hue-rotate-input").value
      }deg)`;
    } else {
      img.style.filter = img.style.filter.replace(/hue-rotate\([^)]*\)/, "");
      img.style.filter += `hue-rotate(${
    ...
    if (!img.style.filter.includes("invert")) {
      img.style.filter += `invert(${
        document.getElementById("invert-input").value / 100
      })`;
    } else {
      img.style.filter = img.style.filter.replace(/invert\([^)]*\)/, "");
      img.style.filter += `invert(${
    ...
    if (!img.style.filter.includes("saturate")) {
      img.style.filter += `saturate(${
        document.getElementById("saturate-input").value / 100
      })`;
    } else {
      img.style.filter = img.style.filter.replace(/saturate\([^)]*\)/, "");
      img.style.filter += `saturate(${
    ...
    if (!img.style.filter.includes("sepia")) {
      img.style.filter += `sepia(${
        document.getElementById("sepia-input").value / 100
      })`;
    } else {
      img.style.filter = img.style.filter.replace(/sepia\([^)]*\)/, "");
      img.style.filter += `sepia(${
  ...
  document.getElementById("original-button").style.backgroundColor =
    generateRGBA(localStorage.getItem("accent"), 0.25);

  document
    .getElementById("original-button")
    .addEventListener("mouseover", () => {
      document.getElementById("original-button").style.backgroundColor =
        generateRGBA(localStorage.getItem("accent"), 0.5);
    });
...
function copyImageURI(uri) {
  ...
  document.getElementById("export-text").innerText =
    "Copied link to filtered image";
  document.getElementById("export-icon").innerText = "check";

  setTimeout(() => {
    document.getElementById("export-text").innerText =
      "Copy link to filtered image";
